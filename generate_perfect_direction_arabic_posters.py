...
def reshape_arabic_raw_reshaped_only(text):
    # PIL with modern TrueType fonts or reshaper alone without BiDi reversal
    reshaper = arabic_reshaper.ArabicReshaper({'delete_harakat': True})
    return reshaper.reshape(text)

def main():
    out_dir = "examples/2026-08-29_arabic_posters_verified_direction"
    ...
    print("Generating 10 Arabic poster samples with verified correct reading direction...")
    for idx, (filename, title, subtitle, c1, c2, accent) in enumerate(posters, start=1):
        ...
        title_render = reshape_arabic_raw_reshaped_only(title)
        sub_render = reshape_arabic_raw_reshaped_only(subtitle)
        date_render = reshape_arabic_raw_reshaped_only("2026-08-29 ❤️")
        ...
        img.save(filepath)
        print(f"[{idx}/10] Saved direction-corrected poster: {filepath}")

    print("All 10 posters generated with correct direction!")
